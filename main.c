...
double gini(double (*)[COUNT_ROWS][3], int, int);
int make_data(double (*)[3], char (*)[LEN]);
double num_cases(int, int);
int count(double*, double, int);

int main(int argc, const char * argv[]) {
    char data[COUNT_ROWS][LEN], path[] = "banknote.csv";
    double converted_data[COUNT_ROWS][3];
    int len;
...
int make_data(double (*data)[3], char (*data_str)[LEN]){
...
double gini(double (*converted_data)[COUNT_ROWS][3], int len1, int len){
